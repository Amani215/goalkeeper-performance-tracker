...
def test_add_calendar(app, category):
    ...
    calendar_service.add_calendar(random_string.generate(4), category.id)

    calendars = category.calendars
    assert len([i.serialize for i in calendars]) == calendar_count + 1


def test_calendar_get_by_id(app, calendar):
    '''Test get a calendar by its ID'''
    _calendar = calendar_service.get_calendar_by_id(calendar.id)
    assert _calendar.id == calendar.id

    # Bad ID
    _calendar = calendar_service.get_calendar_by_id(random_string.generate(7))
    assert 'error' in _calendar


def test_delete_calendar(app, calendar):
    '''Test deleting a calendar'''
    calendar_id = calendar.id
    calendar_service.delete_calendar(calendar_id)

    assert calendar_service.get_calendar_by_id(
